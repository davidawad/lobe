...
class TestBotProcessor(object):
    ...
    def test_extract_reply_from_intent(self):
        response_text = extract_reply_from_intent("greetings")
    ...
    def test_extract_reply_from_intent(self):
        response_text = extract_reply_from_intent("court_basics")
        assert "You will appear before" in response_text

        return


    def test_extract_reply_from_intent_bad_input(self):
        response_text = extract_reply_from_intent(None)
        assert "I'm sorry I didn't understand that!" in response_text
        return

    def test_extract_reply_from_intent_bad_input(self):
        response_text = extract_reply_from_intent('FAKEFAKE')
        assert "I'm sorry I didn't understand that!" in response_text
        return
